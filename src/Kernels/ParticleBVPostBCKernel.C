...
ParticleBVPostBCKernel::validParams()
{
  ...
  params.addRequiredParam<Real>("K2", "The reaction rate for Butler-Volmer reaction");
                            ...
                            const Real & cs, Real & J, Real & dJdc)
{
  ...
  // The transferred macro ce can step slightly outside (0, Cm) during hard
  // transients; clamp it so the sqrt stays real (no fabs here!), and bound
  // eta so exp() cannot overflow.
  ...
  J = _K2 * sqrt((_Cm - cb) * cb) * (cs * exp(0.5 * eta) - (1 - cs) * exp(-0.5 * eta));

  dJdc = _K2 * sqrt((_Cm - cb) * cb) *
