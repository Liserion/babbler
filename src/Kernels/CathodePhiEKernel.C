...
Real
CathodePhiEKernel::OpenCircuitV(const int & matechoice, Real x)
{
  Real V = 0.0;
...
void
CathodePhiEKernel::BV(const Real & c, const Real & phi1, const Real & phi2, const Real & cs,
...
Real
CathodePhiEKernel::computeQpResidual()
{
...
Real
CathodePhiEKernel::computeQpJacobian()
{
  BV(_couple_c[_qp], _couple_phi1[_qp], _u[_qp], _couple_cs[_qp], Jeff, dJdc, dJdphi1, dJdphi2);
...
Real
CathodePhiEKernel::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real t0 = 0.0107907 + _couple_c[_qp] * 1.48837e-4;
  Real dt0 = 1.48837e-4;
